...
def check_and_install_models(force=False, load_only_lang_codes=None, update=False):
    # Load custom models if path is provided
    custom_model_path = os.environ.get("CUSTOM_MODEL_PATH")
    if custom_model_path and os.path.isdir(custom_model_path):
        model_files = glob.glob(os.path.join(custom_model_path, "*.argosmodel"))
        for model_file in model_files:
            try:
                package.install_from_path(model_file)
                print(f"Installed custom model: {os.path.basename(model_file)}")
            except Exception as e:
                print(f"Error installing model {model_file}: {str(e)}")
    
    # Continue with normal model installation if needed
